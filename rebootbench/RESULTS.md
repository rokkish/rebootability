...
## Phase 0.6: docker vs podman runtime 比較

同じ `nginx:alpine` を docker と podman (rootless, 3.4.4) で起動し、同じ injector
モード (`kill-start`, delay=200ms) と同じ cooldown でスイープして比較。

### 補足: podman の `restart` モードが使えない

`podman restart -t 0` は rootless 環境で **`rootlessport listen tcp 0.0.0.0:18080:
bind: address already in use`** で失敗する。古い rootlessport がポートを解放する
前に新しいプロセスがバインドしようとする既知の競合。**`kill-start` モードに
明示的な delay (200ms)** を入れることで回避できる。docker の `restart` モードは
比較対象外。

### 結果 (20 trial × 2 cooldown)

| runtime | cooldown | 完走率 | 復活 avg (ms) | kill CLI (ms) | start→200 (ms) |
|---|---|---:|---:|---:|---:|
| docker | 2s | 13/20 = 65% | 1876 | 1114 (incl. 200 delay) | 762 |
| docker | 5s | 19/20 = 95% | 1897 | 1229 (incl. 200 delay) | 668 |
| podman | 2s | **20/20 = 100%** | 1020 | 501 (incl. 200 delay) | 519 |
| podman | 5s | **20/20 = 100%** | 1009 | 685 (incl. 200 delay) | 324 |

(`kill CLI` 列は `start_at - inject_at`、すなわち `<runtime> kill` の往復時間 + delay。
delay=200ms を差し引いた純粋な kill CLI は docker ~900-1030ms / podman ~300-485ms。)

### 観察

1. **podman は復活時間が約半分** (~1.0s vs ~1.9s)。delay の 200ms を引いても
   ~0.8s vs ~1.7s で 2 倍速。
2. **kill CLI 段階で 2-3 倍の差**: docker は daemon への RPC + 状態管理が走る。
   podman は fork-exec モデルで CLI が自分で OCI runtime (crun) を呼ぶだけ。
3. **start→200 段階でも 2 倍の差**: rootless podman の slirp4netns は遅いと
   思われがちだが、port forwarder 再構築は (delay=200ms 入れる限りでは) むしろ
   速い。docker-proxy の方が重いか daemon の状態反映に時間が要るか。
4. **podman は cooldown=2s でも 100% 完走**。docker は cooldown=5s でも 95% 止まり。
   docker daemon の連続再起動耐性が podman より弱い。

### 切り分けと含意

- **「nginx の再起動コスト」と思っていた数字の大半は runtime overhead だった**。
  Phase 0 で「真の recovery は < 10ms」と仮説を立てて 1.5s と修正したが、その
  1.5s すら半分以上が runtime CLI/daemon の時間。SUT (nginx) 自体の起動は
  おそらく 100-200ms オーダー。
- 「同じワークロードでも runtime を変えると 2 倍速くなる」は、レジリエンス
  ベンチマークが SUT 単体評価ではなく **SUT × ランタイム × 設定の合成**を測って
  いることの典型例。plan の 4 軸スコア化で各次元を独立に評価するモチベが補強
  された。
- podman は rootless でこれ。**rootful podman / Docker ではない選択肢**が
  Phase 1 でも検討に値する。systemd + podman の組合せは k8s 抜きで auto-restart
