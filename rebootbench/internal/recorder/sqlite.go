...
func (r *Recorder) SaveTrial(t TrialRow) error {
	var firstRec, recNs interface{}
	if !t.FirstRecoveryAt.IsZero() {
		firstRec = t.FirstRecoveryAt.UnixNano()
		recNs = int64(t.RecoveryTime)
	}
	_, err := r.db.Exec(
		`INSERT OR REPLACE INTO trial (experiment_id, trial_index, inject_at, first_recovery_at, recovery_time_ns, status) VALUES (?, ?, ?, ?, ?, ?)`,
		t.ExperimentID, t.Index, t.InjectAt.UnixNano(), firstRec, recNs, t.Status,
	)
	return err
}
