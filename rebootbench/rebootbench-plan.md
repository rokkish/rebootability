...
#### 2.1 k8s pod restart 計測 (Phase 0.7 4 層比較の延長)

bare → OCI runtime → container CLI → daemon の 4 層は計測済み (RESULTS.md)。
**5 層目: kubernetes pod restart** を加えると「orchestrator がさらに何 ms
乗せるか」が定量化できる。Phase 2 のサブゴール。

**設計**:
- 環境: kind か k3s (シングルノード k8s)。systemd 上の rootful podman でも
  代替可。
- SUT: 同じ tinyserver image (`rebootbench/tinyserver:latest`)。Service
  (NodePort or ClusterIP via port-forward) で probe URL を露出。
- Injector: `K8sDeletePod`
  - `kubectl delete pod <name> --grace-period=0 --force`
  - Deployment / ReplicaSet が新 pod を作成、Service が新 endpoint を pickup
  - **回復定義**: probe が 200 を返すまで (Service の readiness 反映を含む)
- 期待される overhead (Phase 0.7 の比較からの予想):
  - container CLI overhead (~800ms, podman 由来) +
    k8s scheduling / endpoints update / kubelet→runtime 経路 (推定 +1〜2秒)
  - 合計 2〜4 秒程度を予想
- 切り分けたい変数:
  - pod の startupProbe / readinessProbe 設定 (これが endpoint 反映を律速)
